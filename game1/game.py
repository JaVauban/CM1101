...
def normalise_input(user_input):
    ...
    'help'
    """
    return re.sub(r"[^\d\s\w]", "", user_input.lower()).strip()
...
def display_room(room):
    ...
    #NOTE: Using print(string, end='')
    #      will print to the same line
    #Thusly, adding an extra newline char should cause it to do 2 newlines
    print("")
...
def print_menu_line(direction, leads_to):
    ...
    """
    #Not really necessary given the doctests if(is_valid_exit(, direction)):
...
def print_menu(exits):
    ...
    for dir in directions:
        if(is_valid_exit(exits, dir)):
            #print(rooms[exits[dir]].get("name"))
            print("Go " + dir.upper() + " to " + exit_leads_to(exits, dir) + ".") 
...
def move(exits, direction):
    ...
    False
    """
    # Given that Python has no pointers or refererences at all, I have 
    # to hope that <room>["name"] is unique
